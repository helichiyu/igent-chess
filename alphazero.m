...
for d = 1:draw
    ...
    % 获取行动
    action = drop(game,net,c,ndata);k2 = {action};
    % 将行动存入字典
...
function action = drop(game,net,c,ndata)
% 找到我方棋子
pieces = findOurPieces(game);
% 初始化参数（s分数，k1元胞数组，sumn总选择次数，nk该局面对于的map）
k1 = {game};sumn = 0;nk = ndata(k1);
if nk.numEntries ~= 0
    ks = keys(nk);
    % 计算之前已经选择了多少次
    for i = 1:length(ks)
...
function draw_progress_bar(iter, max_iter, lun, max_lun)
    ...
    fff = 0;
    if lun == 10 || lun == 100 || lun == 1000 || lun == 10000
         fff = fff + 1;
    end
    if percent == 0.1 || percent == 1
        fff = fff + 1;
    end
    backspaces = repmat(sprintf('\b'), 1, length(str)-fff);
    % 输出进度条和迭代信息
    if lun ~= 1
